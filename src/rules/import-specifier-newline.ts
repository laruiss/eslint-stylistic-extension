...
const rule: Rule.RuleModule = {
  meta: {
    ...
    docs: {
      description:
        "Enforce line breaks between named specifiers of an import or export declaration",
    },
    fixable: "whitespace",
    schema: [
      {
    ...
    function checkSpecifiers(specifierNodes: any[], checker: Checker) {
      for (let i = 1; i < specifierNodes.length; i++) {
        ...
        if (checker.check(prevSpecifierToken, currentSpecifierToken)) {
          const tokenBefore = sourceCode.getTokenBefore(currentSpecifierToken, {
          ...
          context.report({
            ...
            fix: hasLineCommentBefore
              ? null
              : checker.createFix(currentSpecifierToken, tokenBefore),
          });
        }
      }
    }
